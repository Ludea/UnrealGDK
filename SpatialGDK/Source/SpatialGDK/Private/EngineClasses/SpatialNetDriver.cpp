...
void USpatialNetDriver::CreateAndInitializeCoreClasses()
{
	...
	if (WorkerType == SpatialConstants::DefaultServerWorkerType || WorkerType == SpatialConstants::DefaultClientWorkerType)
	{
		...
		const SpatialGDK::FSubView& ActorAuthSubview = Connection->GetCoordinator().CreateSubView(
			SpatialConstants::ACTOR_AUTH_TAG_COMPONENT_ID, ActorFilter, TombstoneRefreshCallbacks);
		...
		ActorSystem = MakeUnique<SpatialGDK::ActorSystem>(ActorNonAuthSubview, TombstoneActorSubview, this, &TimerManager,
...
void USpatialNetDriver::ProcessRPC(AActor* Actor, UObject* SubObject, UFunction* Function, void* Parameters)
{
	...
	if (Info.Type == ERPCType::CrossServer)
	{
		...
		{
			return CrossServerRPCSender->SendCommand(MoveTemp(CallingObjectRef), CallingObject, Function, MoveTemp(Payload), Info, {});
		}
		else
		{
